...
class SCPMAnalyzer(BaseAnalyzer):
    ...
    def _count_components(self, adjacency: Dict[str, Set[str]], methods: List[str]) -> int:
        """
        Count disconnected components in the method graph using DFS.
        
        LCOM analysis: If there are X disconnected components (X > 1),
        it suggests the module should be split into X separate modules.
        ...
        def dfs(method: str):
            """Depth-first search to mark all connected methods."""
            visited.add(method)
            for neighbor in adjacency[method]:
    ...
    def _get_files_accessed(self, method_node: ast.FunctionDef) -> Set[str]:
        """
        Extract data/model/config file paths accessed in a method.
        
        Looks for string literals with ML-related file extensions:
        - Data: .csv, .json, .parquet, .xlsx, etc.
        - Models: .pkl, .h5, .pt, .ckpt, etc.
        - Configs: .yaml, .yml, .ini, .cfg, etc.
        
        Args:
            method_node: AST node of the method/function
            
        Returns:
            Set of file paths accessed
        """
        files = set()
        ...
        for node in ast.walk(method_node):
            ...
            if isinstance(node, ast.Constant) and isinstance(node.value, str):
                ...
                if any(file_path.endswith(ext) for ext in self.ml_file_extensions):
                    # Normalize path (extract basename for comparison)
    ...
    def _is_likely_global_variable(self, var_name: str) -> bool:
        """
        Enhanced heuristic to determine if a variable is likely a global.
        ...
        # Pattern: Starts with uppercase but is not in ml_types
